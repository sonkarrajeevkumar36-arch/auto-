...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('switch-mode').onclick = () => {
        ...
        document.getElementById('switch-mode').innerText = title.innerText === "Login" ? "New? Register" : "Have an account? Login";
    };
    ...
    async function loadUserOrders() {
        ...
        try {
            ...
            list.innerHTML = snap.docs.map(doc => {
                const o = doc.data();
                return `<div style="border-bottom:1px solid #eee; padding:10px; margin-bottom:5px;">
                            <div style="display:flex; justify-content:space-between;">
                                <b>Total: ₹${o.total}</b>
                                <span class="status-tag status-${o.status}">${o.status}</span>
                            </div>
                            <small>Items: ${(o.items || []).length}</small>
                        </div>`;
            }).join('');
        } catch (e) {
            list.innerHTML = "<p>Orders load karne mein error aaya. (Firestore Index required)</p>";
            console.error(e);
        }
    }
});